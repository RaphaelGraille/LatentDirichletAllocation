...
std::vector<unsigned int> treat_transformed_clustering(const std::string &str){
	const char* filename = str.c_str();
    std::ifstream file(filename);
    std::string line;
    std::vector<unsigned int> res;
...
void results(const std::string &cluster, const std::string &corpus, char* file_name) {
	...
	//std::vector<unsigned int>predicted_classes = treat_transformed_clustering(cluster);
...
/*	unsigned int i =0;
	for (unsigned int k=1; k<=nbClasses; k++) {
		while (expert_classes[i] == k && i<expert_classes.size()) {
			repartition[k-1][predicted_classes[i]-1] += 1;
			classes_size[k-1] += 1;
